...
// 列削除・並び替え後にデータごと正規列順へ復元する
function restoreSheetColumnOrder_(sheet, canonHdr) {
  var lastRow = sheet.getLastRow();
  var lastCol = sheet.getLastColumn();
  if (lastCol === 0) {
  ...
  canonHdr.forEach(function(name) {
    ...
    for (var r = 0; r < allData.length; r++) {
      newData[r].push(src >= 0 ? allData[r][src] : (r === 0 ? name : ''));
    }
  ...
  try {
    ...
    try {
      ...
      names.forEach(function(name) {
        ...
        if (bad) { restoreSheetColumnOrder_(s, canon); restored = true; }
      });
