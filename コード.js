...
function normalizeDocDateTime_(rawVal, now) {
  if (rawVal instanceof Date && !isNaN(rawVal.getTime())) {
    var dv = rawVal;
    if (dv.getFullYear() < 1902) {
      // Sheets時刻専用epoch (Date(1899-12-30 HH:mm)) → 今日の日付 + その時刻
      dv = new Date(now.getFullYear(), now.getMonth(), now.getDate(), dv.getHours(), dv.getMinutes(), 0);
    } else if (dv.getHours() === 0 && dv.getMinutes() === 0 && dv.getSeconds() === 0) {
      // 日付のみ入力（0:00:00） → その日付 + 現在時刻
      dv = new Date(dv.getFullYear(), dv.getMonth(), dv.getDate(), now.getHours(), now.getMinutes(), 0);
    }
    return Utilities.formatDate(dv, 'Asia/Tokyo', 'MM/dd HH:mm');
  }
  if (typeof rawVal === 'string' && rawVal.trim() !== '') {
    var ds = rawVal.trim()
      .replace(/[：]/g, ':').replace(/[　]/g, ' ')
      .replace(/[０-９]/g, function(c){ return String.fromCharCode(c.charCodeAt(0) - 0xFEE0); });
    var dm = ds.match(/^(\d{1,2})\/(\d{1,2})\s+(\d{1,2}):(\d{2})/);
    if (dm) {
      var d1 = new Date(now.getFullYear(), parseInt(dm[1])-1, parseInt(dm[2]), parseInt(dm[3]), parseInt(dm[4]), 0);
      if (!isNaN(d1.getTime())) return Utilities.formatDate(d1, 'Asia/Tokyo', 'MM/dd HH:mm');
    }
    var dm1d = ds.match(/^(\d{1,2})\/(\d{1,2})$/);
    if (dm1d) {
      var d2 = new Date(now.getFullYear(), parseInt(dm1d[1])-1, parseInt(dm1d[2]), now.getHours(), now.getMinutes(), 0);
      if (!isNaN(d2.getTime())) return Utilities.formatDate(d2, 'Asia/Tokyo', 'MM/dd HH:mm');
    }
    var dm2 = ds.match(/^(\d{1,2}):(\d{2})$/);
    if (dm2) {
      var d3 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), parseInt(dm2[1]), parseInt(dm2[2]), 0);
      if (!isNaN(d3.getTime())) return Utilities.formatDate(d3, 'Asia/Tokyo', 'MM/dd HH:mm');
    }
  }
  return '';
}
...
function applyHolidayRowColors_(ss) {
  ...
  if (sheet && sheet.getLastRow() >= 2) {
    ...
    var bgs2D   = vals.map(function(r, idx) {
      ...
      // 通常行: 既存背景を保護し、有休/休み色・保護色・IDなし行の期限警告残骸をクリア
      var rowArr = curBgs[idx].slice();
      for (var ci = 0; ci < rowArr.length; ci++) {
        var _c = rowArr[ci];
        if (_c === '#e0e0e0' || _c === '#9e9e9e' || _c === '#eceff1') { rowArr[ci] = null; continue; }
        // IDなし行の期限警告残骸（赤/青/緑/濃赤）を除去
        if (!idV && (_c === '#ffcdd2' || _c === '#bbdefb' || _c === '#c8e6c9' || _c === '#ff1744')) rowArr[ci] = null;
      }
      if (pickV === '' && idV !== '') {
      ...
      for (var tr = 0; tr < tailBgs.length; tr++) {
        for (var tc = 0; tc < tailBgs[tr].length; tc++) {
          ...
          if (tv === '#eceff1' || tv === '#e0e0e0' || tv === '#9e9e9e' || tv === '#fff9c4' ||
              tv === '#ffcdd2' || tv === '#bbdefb' || tv === '#c8e6c9' || tv === '#ff1744') {
            tailBgs[tr][tc] = null;
            tailDirty = true;
          }
        }
...
function sortUnkouByDate_(companySsId, skipBlankDelete) {
  ...
  if (!skipBlankDelete && blankRows.length > 0) deleteRowsGrouped_(sheet, blankRows);
}
...
function onOpen(e) {
  ...
  menu
    ...
    .addSubMenu(ui.createMenu('📥 データ読み込み（CSV）')
      .addSubMenu(ui.createMenu('🚛 運行シート')
        .addItem('📋 貼付シート作成',               'createPasteImportSheetUnkou')
        .addItem('▶ 貼付データ取込',                'executePasteImportUnkou')
        .addItem('✅ この内容で確定・反映',           'confirmPasteImport'))
      .addSubMenu(ui.createMenu('🗄 自車専属マスタ')
        .addItem('📋 貼付シート作成',               'createPasteImportSheetMaster')
        .addItem('▶ 貼付データ取込',                'executePasteImportMaster')
        .addItem('✅ この内容で確定・反映',           'confirmPasteImport'))
      .addSubMenu(ui.createMenu('📇 マスタ（取引先）')
        .addItem('📋 貼付シート作成',               'createPasteImportSheetCust')
        .addItem('▶ 貼付データ取込',                'executePasteImportCust')
        .addItem('✅ この内容で確定・反映',           'confirmPasteImport'))
      .addSeparator()
    ...
    .addSubMenu(ui.createMenu('⚙️ システム設定・保守')
      ...
      .addItem('📗 解説書作成',                 'createManualMASheet')
...
function onEdit(e) {
  try {
    ...
    // ── 3-1-1: 集計表 編集ブロック ──────────────────────────────────
    // 距離(W=23)・ガソリン代(Y=25)・支払い(AA=27)・備考(AD=30)・その他手当(AI=35)以外の列は直接編集を禁止する
...
function onEditUnkou_(sheet, range, ss) {
  ...
  // 帳票発行日時列（発注書・指示書/車番連絡/受領書）の列番号を動的取得
  var _docColNums = [];
  if (editedEndCol >= 23) {
    var _hdrs0 = _unkHdr0 || sheet.getRange(1, 1, 1, lastColU).getValues()[0];
    ['発注書・指示書', '車番連絡', '受領書'].forEach(function(name) {
      var ci = _hdrs0.indexOf(name);
      if (ci >= 0) _docColNums.push(ci + 1);
    });
  }
  ...
  for (var i = 0; i < numRows; i++) {
    ...
    if (editedCol <= 10 && editedEndCol >= 10) {
      ...
      if (dateVal instanceof Date) {
        var now = new Date();
        if (dateVal.getFullYear() < 1902) {
          // Sheets時刻専用epoch → J列（運行日）では時刻のみ入力は無効。現在日付に置き換え
          dateVal = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0);
          sheet.getRange(row, 10).setValue(dateVal);
          rowData[9] = dateVal;
        } else if (dateVal.getHours() === 0 && dateVal.getMinutes() === 0 && dateVal.getSeconds() === 0) {
    ...
    // 帳票発行日時列（発注書・指示書/車番連絡/受領書）：入力値を MM/dd HH:mm テキスト形式に正規化
  ...
  if (status === '運行') {
    ...
    try {
      ...
      for (var day = startDay; day <= endDay; day++) {
        ...
        formulas.push(['=IF(U'+rn+'=T'+rn+',"",T'+rn+'-U'+rn+')']);
      }
      if (genNext) {
        for (var nday = 1; nday <= nxtEnd; nday++) {
          ...
          formulas.push(['=IF(U'+nrn+'=T'+nrn+',"",T'+nrn+'-U'+nrn+')']);
        }
      }
...
function expandAndRefreshSheets() {
  ...
  // 運行・自車専属マスタのトン数・車種をonEditと同じ形式に一括正規化
  ['運行', '自車専属マスタ'].forEach(function(sname) {
    var ns = ss.getSheetByName(sname);
    if (!ns || ns.getLastRow() < 2) return;
    var nLastCol = ns.getLastColumn();
    var nHdrs    = ns.getRange(1, 1, 1, nLastCol).getValues()[0];
    var tIdx     = nHdrs.indexOf('トン数');
    var cIdx     = nHdrs.indexOf('車種');
    if (tIdx < 0 && cIdx < 0) return;
    var nData    = ns.getRange(2, 1, ns.getLastRow() - 1, nLastCol).getValues();
    var chg      = false;
    nData.forEach(function(row) {
      if (tIdx >= 0) {
        var nt = normalizeTons_(String(row[tIdx] || '').trim());
        if (nt !== String(row[tIdx] || '')) { row[tIdx] = nt; chg = true; }
      }
      if (cIdx >= 0) {
        var cv  = String(row[cIdx] || '');
        var nct = cv.replace(/[Ａ-Ｚａ-ｚ]/g, function(c){return String.fromCharCode(c.charCodeAt(0)-0xFEE0);}).toUpperCase();
        if (nct !== cv) { row[cIdx] = nct; chg = true; }
      }
    });
    ...
    if (phRowNum !== -1) {
      ...
      if (picks.length > 1) {
        ...
        for (var af = 0; af < addData.length; af++) {
          var afr = addStart + af;
          addFmls.push(['=IF(U'+afr+'=T'+afr+',"",T'+afr+'-U'+afr+')']);
        }
      ...
      for (var fi = 0; fi < num; fi++) {
        var fr = startRow + fi;
        formulas.push(['=IF(U'+fr+'=T'+fr+',"",T'+fr+'-U'+fr+')']);
      }
...
function createManualSheet() {
  ...
  mainTitle('運行管理システム　操作説明書（現場スタッフ向け）');
  sp();
  ...
  feature('4', '車種 全角→半角・大文字統一変換',
    '【入力時・自動】\nE列（車種）に小文字や全角アルファベット（「truck」「Ｗ翼」等）で入力しても自動で大文字・半角に変換');
  feature('5', '車番 全角→半角自動変換',
    '【入力時・自動】\nF列（車番）に「１０１」など全角英数で入力しても自動で半角「101」に変換して書き戻す');
  feature('6', '日付への現在時刻付与',
    '【入力時・自動】\nJ列（日付）に「7/24」など日付のみ入力すると自動で現在時刻を付与して保存（例：「7/24」→「2026/07/24 14:35」）');
  feature('7', '時刻の正規化と日付合成',
    '【入力時・自動】\nN〜R列等の時刻入力を自動整形。全角コロン「：」・全角数字を半角に変換。時刻のみ入力するとJ列の日付と自動合成して統一');
  sp();

  catHead('【自動計算】');
  feature('8', '書式自動設定',
    '【入力時・自動】\n売上・請求高速・実費高速・合計高速列にカンマ区切りの数値書式を自動設定。時刻列に時刻書式を自動設定・修復');
  feature('9', '請求高速→実費高速の自動コピー',
    '【入力時・自動】\nT列（請求高速代）に入力するとU列が空であればオレンジ文字で同じ値を自動コピー。U列を直接手入力すると黒字に戻り自動コピーが解除。U列をDeleteで空にするとT列の値が再セット\n実費が本当に0円の場合は「0」と明示入力すること');
  feature('10', '合計高速代の数式自動セット',
    '【入力時・自動】\nT列またはU列を編集するとV列（合計高速）に数式が自動セット（T−U）。V列を直接編集しようとすると数式が書き戻される');
  feature('11', '日付変更時の自動ソート',
    '【入力時・自動】\nJ列（日付）を変更すると運行シートと集計表が「日付順→同じ日は乗務員名の五十音順」に自動並び替え\n\n【メニュー操作】\n「毎日の配車業務」→「🔃 日付順並び替え」で手動実行も可能');
  sp();
  ...
  feature('18', '配車漏れ警告色',
    '【入力時・自動】\nL列（積地）が空でA列（ID）がある行のL列が薄黄（#fff9c4）で着色。積地を入力すると即時解除');
  feature('19', '保護列の色（薄青グレー）',
    '【入力時・自動】\nV列（合計高速代）・Y列・Z列のIDがある行に薄青グレー（#eceff1）を自動適用。手で入力しない列であることを視覚的に表示');
  feature('20', '資格期限アラート色（A列）',
    '【SS起動時・自動】\n期限超過=淡い赤（#ffcdd2）、当日=淡い青（#bbdefb）、7日以内=淡い緑（#c8e6c9）でA列を着色');
  feature('21', '色凡例メモの自動設定',
    '【SS起動時・自動】\n運行シートA1セル・L1セルと自車専属マスタB1セルに色の意味をメモとして自動書き込み');
  sp();

  catHead('【バックグラウンド】');
  feature('22', 'マスタ編集時連動',
    '【入力時・自動】\n自車専属マスタのB列「運行状態」を変更すると「いつから適用しますか？」ポップアップが表示。「待機→運行」変更時は選択日付以降の空行を自動追加。「運行→待機/故障」変更時は配車が入っていない空行を自動削除');
  sp();

  catHead('【配車板】');
  feature('23', '配車ダッシュボード',
    '【メニュー操作】\n「毎日の配車業務」→「🚚 配車ダッシュボード」を開く。本日の運行シートから積地（L列）が空欄の行＝配車未確定行を自動で一覧表示。タップすると対象行へジャンプ');
  feature('24', '選択行の配車確定解除',
    '【メニュー操作】\n対象行にセルを合わせ「🔓 選択行の配車確定解除」を実行。確認ダイアログ後に対象行を運行シート・集計表・配車板からまとめて削除');
  sp();

  catHead('【手動操作】');
  feature('25', 'ID・車番一括補完（手動）',
    '【メニュー操作】\n「毎日の配車業務」→「🆔 ID・車番一括補完」を実行。A列が空の行にIDを一括採番し、補完が抜けた行も車番マスタからまとめて補完');
  feature('26', '写真・ファイル取込',
    '【メニュー操作】\n取込先行にセルを合わせる → 「写真・ファイル取込」を選択 → 右側のサイドバーでファイルを選択（複数選択可）。完了後、X列にGoogleドライブのリンクが追記される');
  feature('27', '運行シートCSV/Excel取込',
    '【手動操作（3ステップ）】\n①「データ読み込み（CSV）」→「運行シート」→「📋 貼付シート作成」で取込用シートを作成\n②作成されたシートにCSV/ExcelデータをSTEP3行目以降に貼り付ける\n③「▶ 貼付データ取込」→「✅ この内容で確定・反映」で運行シートへ書き込み。完了後、取込用シートは自動削除\n重複データ（日付・車番・乗務員名・積地・降地・売上の複合キー）は自動スキップ');
  sp();

  catHead('【帳票・出力】');
  feature('28', '発注書・指示書の作成・送信',
    '【手動操作】\n対象行のIDセルを選択 → 「帳票・送信メニュー」→「① 発注書・指示書を作成」\n「印刷/PDF保存」「メール送信」「FAX送信」から送信方法を選ぶ\nいずれかのボタンを押した瞬間にAD列（発注書・指示書）に発行日時が自動記録');
  feature('29', '車番連絡の作成・送信',
    '【手動操作】\n対象行のIDセルを選択 → 「帳票・送信メニュー」→「② 車番連絡を作成（荷主用）」\nいずれかのボタンを押した瞬間にAE列（車番連絡）に発行日時が自動記録');
  feature('30', '受領書の耳生成',
    '【手動操作】\n「帳票・送信メニュー」→「🗒 受領書の耳生成」を選ぶ\n絞り込みダイアログ（荷主・会社名・日付・車番・乗務員名・積地・降地）で「生成」を押す\n受領書_耳シートが生成され「印刷用PDFを開く」でA4縦PDFが開く');
  feature('31', '請求書生成',
    '【メニュー操作】\n「📒 経理・出力」→「🧾 請求書生成」を選ぶ\n取引先・期間・消費税率を選んで「生成」を押す\n4ドロップダウン（マスタID・取引先名カナ・会社名・電話）はいずれかを選ぶと残り3つが自動連動\n対象期間の運行データを集計し売上・高速代・消費税・合計金額を自動計算した請求書シートを生成');
  feature('32', 'CSV・Excel出力',
    '【メニュー操作】\n「📒 経理・出力」→「💾 CSV・Excel出力」を選ぶ\n各シート右のCSVボタンでBOM付きUTF-8 CSVをダウンロード\nチェックボックスでExcel出力対象シートを選択し「📊 選択シートをExcel DL」でXLSXダウンロード');
  sp();
...
function createManualMASheet() {
  ...
  function feature(num, name, code, impl) {
    sheet.getRange(row,2).setValue(num + '  ' + name + '\n' + code)
      .setBackground('#eef1f3').setFontSize(10).setFontWeight('bold')
      .setVerticalAlignment('top').setWrap(true);
    sheet.getRange(row,3).setValue(impl)
      .setFontSize(10).setVerticalAlignment('top').setWrap(true)
      .setBackground('#ffffff');
    sheet.setRowHeight(row, 72); row++;
  }
  ...
  mainTitle('運行管理システム　機能解説書（エンジニア向け）');
  sp();
  ...
  feature('4', '車種 全角→半角・大文字統一変換', 'コード.js 3-2（onEditUnkou_）',
    'E列編集時に全角アルファベットを半角化しtoUpperCase。F列自動補完時もマスタ側の車種を同様に変換して書き戻し');
  feature('5', '車番 全角→半角自動変換', 'コード.js 3-2（onEditUnkou_）',
    'F列編集時に全角英数字（０-９Ａ-Ｚ等）を半角に変換してsetValue。照合は数字部分の正規表現完全一致');
  feature('6', '日付への現在時刻付与', 'コード.js 3-2（onEditUnkou_）',
    'J列編集時、入力値が時刻0:00:00で日付のみと判定されたとき、new Date()で現在時刻を取得してsetValue');
  feature('7', '時刻の正規化と日付合成', 'コード.js 3-2（onEditUnkou_）',
    'N〜R列等の時刻列編集時、全角コロン・全角数字を半角変換。時刻のみ入力（時刻+日付なし）の場合はJ列の日付と合成してDateオブジェクトを生成しsetValue');
  sp();

  catHead('【自動計算】');
  feature('8', '書式自動設定', 'コード.js 1-5（applyMoneyFormat_）・1-6（applyDateTimeFormat_）・3-2（onEditUnkou_）',
    'applyMoneyFormat_：S〜V列に#,##0書式をsetNumberFormat\napplyDateTimeFormat_：N〜R列等に「M/d HH:mm」書式をsetNumberFormat\nonEdit時に対象列が含まれる場合に両関数を呼び出し');
  feature('9', '請求高速→実費高速の自動コピー', 'コード.js 3-2（onEditUnkou_）',
    'T列編集時にU列が空であれば同値をsetValueしfontColorをorangeに設定。U列を直接編集するとfontColorをnullに戻し自動コピーを解除。U列をDeleteするとT列の値を再セット');
  feature('10', '合計高速代の数式自動セット', 'コード.js 3-2（onEditUnkou_）',
    'T列またはU列編集時にV列の数式を確認。数式なしの場合「=T{n}-U{n}」をsetFormula\nV列直接編集時は即座に数式を書き戻し「合計高速は自動計算列です」トーストを表示');
  feature('11', '日付変更時の自動ソート', 'コード.js 1-8（sortUnkouByDate_）・1-9（sortSummaryByDate_）・3-2（onEditUnkou_）',
    'J列編集時にsortUnkouByDate_（運行シート）とsortSummaryByDate_（集計表）を呼び出し。行背景色・ファイルリンクも一緒に移動するためリッチコンテンツ対応の並び替えを実装');
  sp();
  ...
  catHead('【色表示】');
  feature('16', '有休行の色付け', 'コード.js 1-7（applyHolidayRowColors_）・3-2（onEditUnkou_）',
    'L列編集時にapplyHolidayRowColors_を呼び出し。「有休」を含む値なら行全体を#e0e0e0でsetBackground\n優先順位：有休グレー > 資格期限アラート色（赤・青・緑）');
  feature('17', '休み行の色付け', 'コード.js 1-7（applyHolidayRowColors_）・3-2（onEditUnkou_）',
    'L列に「休み」を含む値なら行全体を#9e9e9eでsetBackground\n優先順位：休みグレー > 資格期限アラート色');
  feature('18', '配車漏れ警告色', 'コード.js 1-7（applyHolidayRowColors_）・3-2（onEditUnkou_）・2-1（onOpen）',
    'L列が空でA列（ID）が存在する行のL列セルを#fff9c4でsetBackground\nL列に値が入ると即時クリア。有休・休み行には上書きしない');
  feature('19', '保護列の色（薄青グレー）', 'コード.js 1-7（applyHolidayRowColors_）・3-2（onEditUnkou_）・2-1（onOpen）',
  ...
  catHead('【バックグラウンド】');
  feature('22', 'マスタ編集時連動', 'コード.js 3-3（onEditMasterVehicle_）・3-3b（syncVehicleToCurrentMonth_）',
    'onEditMasterVehicle_：自車専属マスタB列（運行状態）編集時に起動。「本日以降」「今月以降」「全期間」「キャンセル」の4択ポップアップを表示\nsyncVehicleToCurrentMonth_：選択期間に応じて空行を追加または配車なし空行を削除');
  sp();

  catHead('【配車板】');
  feature('23', '配車ダッシュボード', 'コード.js 2-5（showDispatchDashboard）・dispatchDashboard.html',
    'showDispatchDashboard：当日日付の運行シートからL列（積地）が空でA列にIDがある行を抽出してHtmlServiceでサイドバー表示。行クリックでスプレッドシートの対象行へジャンプ');
  feature('24', '選択行の配車確定解除', 'コード.js 15-1b（cancelDispatch）',
    'cancelDispatch：選択行のA列IDを読み取り、運行シート・集計表・配車板から同IDの行を削除。確認ダイアログ後に実行');
  sp();

  catHead('【手動操作】');
  feature('25', 'ID・車番一括補完（手動）', 'コード.js 4-2b（fillMissingIdsAndCars）',
  ...
  catHead('【帳票・出力】');
  feature('28', '発注書・指示書の作成・送信', 'コード.js 14-2c（showHatchuDocDialog）・14-2e（getDocumentData_）・14-4（markDocumentIssued）・14-5（sendDocumentEmail）',
    'showHatchuDocDialog：HtmlServiceダイアログでgetDocumentData_から運行データを取得し発注書フォームを表示\nmarkDocumentIssued(rowId, docType, ssId)：openById(ssId)で運行シートのAD列に発行日時を記録（ssId引数必須・スタブ経由の引数3つ）');
  feature('29', '車番連絡の作成・送信', 'コード.js 14-2d（showShabanDocDialog）・14-4（markDocumentIssued）・14-11（getKyoryokuHistory）・14-12（saveKyoryokuHistory）',
    'showShabanDocDialog：発注書と同様の構造。markDocumentIssuedでAE列に発行日時を記録\n協力会社履歴：隠しシート「協力会社履歴」にgetKyoryokuHistory/saveKyoryokuHistoryで読み書き');
  feature('30', '受領書の耳生成', 'コード.js 16-1（showUketorishoDialog）・16-2（generateUketorishoSheet）',
    'generateUketorishoSheet（16-2）：フィルタ条件で行を抽出・日付昇順ソート後、3列×7行=21件/ページのレイアウトを生成してシートに書き込み');
  feature('31', '請求書生成', 'コード.js 16-3（showInvoiceDialog）・16-3b（generateInvoiceBatch）・16-6（generateInvoiceSheet）',
    'showInvoiceDialog：4ドロップダウン（マスタID・取引先名カナ・会社名・電話）の連動はonChange時に残り3つをserver-side関数で更新\ngenerateInvoiceBatch：対象期間の運行データを集計しgenerateInvoiceSheetでシート生成');
  feature('32', 'CSV・Excel出力', 'コード.js 17-6b（showExportDialog）・17-6b-1（exportSheetAsCsvBase64）・17-6b-2（exportSelectedSheetsAsExcel）',
    'exportSheetAsCsvBase64：シートをCSV文字列化してBOM付きBase64でフロントへ返却\nexportSelectedSheetsAsExcel：Drive.Files.copyとSheets APIでXLSX形式に変換してBase64で返却');
  sp();
  ...
  sheet.setFrozenRows(0);
  sheet.setTabColor('#2c4356');
  ui.alert('「解説書」シートを作成しました（1〜34番）。');
}
...
function deployClientWebApp_(ssId, companyName, existingScriptId, libVersion) {
  try {
    ...
    // スクリプトID確定（引数で渡された既存ID → Drive列挙で既存確認 → なければ新規作成）
    var scriptId = existingScriptId || null;
    if (!scriptId) {
      // 既存スクリプトがあれば使い回す（新規作成で幽霊を増やさない）
      var existingMap = buildSsScriptMap_();
      var existingIds = existingMap[ssId] || [];
...
function initClientSSSheets_(ss, companyName) {
  ...
  var custHeader    = ['マスタID','取引先名カナ','会社名','電話','FAX','郵便番号','住所','代表者','配車担当','銀行名','支店名','種別','番号','名義','備考','メールアドレス','入金サイクル','入金サイト','受領書送付先郵便番号','受領書送付先住所','インボイス登録番号','インボイス発行者名（自社名）'];
...
function dispatchInstalledEdit(e) {
  try {
    ...
    if (sheetName === '運行' && row >= 2) {
      ...
      // 帳票発行日時列（発注書・指示書/車番連絡/受領書）：MM/dd HH:mm テキスト形式に正規化
      if (col >= 23) {
        var _dHdrs = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
        var _dCols = ['発注書・指示書', '車番連絡', '受領書'].map(function(n){ return _dHdrs.indexOf(n) + 1; }).filter(function(c){ return c > 0; });
        if (_dCols.indexOf(col) !== -1) {
          var _dNumRows2 = range.getNumRows();
          var _dNow2 = new Date();
          for (var _dr2 = 0; _dr2 < _dNumRows2; _dr2++) {
            if (row + _dr2 < 2) continue;
            var _dCell2 = sheet.getRange(row + _dr2, col);
            var _dNorm2 = normalizeDocDateTime_(_dCell2.getValue(), _dNow2);
            if (_dNorm2) { _dCell2.setNumberFormat('@'); _dCell2.setValue(_dNorm2); }
          }
        }
      }
    ...
    if (_pasteNames.indexOf(sheetName) !== -1 && row === 2) {
      ...
      if (_newLbl && _newLbl !== '-') {
        var _origHdr = String(sheet.getRange(1, col).getValue()).trim();
        if (_origHdr && _origHdr !== '-') {
          var _pType = PropertiesService.getDocumentProperties().getProperty('PASTE_IMPORT_TYPE') || 'unkou';
          addImportDictionaryAlias_(e.source, _pType, _newLbl, _origHdr);
          e.source.toast('辞書に追加: 「' + _origHdr + '」→「' + _newLbl + '」', '📚 辞書更新', 3);
        }
      }
      return null;
    }
...
function ensureRequiredSheets(masterSsId) {
  ...
  var masterSs = null;
  try { if (masterSsId) masterSs = SpreadsheetApp.openById(masterSsId); } catch(e) {}
  needed.forEach(function(n) {
    if (ss.getSheetByName(n)) return;
    try {
      var src = masterSs ? masterSs.getSheetByName(n) : null;
      if (src) { src.copyTo(ss).setName(n); } else { ss.insertSheet(n); }
    } catch(e) {}
  });
}
...
function syncToAllClientSS() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      // masterSsId を __COMPANY_SS__ B1 に記録し、非表示にする（システムシート・客には不要）
      var _cmpSh = clientSs.getSheetByName('__COMPANY_SS__');
      if (_cmpSh) {
        try { _cmpSh.getRange(1, 2).setValue(masterSsId); } catch(e) {}
        try { if (!_cmpSh.isSheetHidden()) _cmpSh.hideSheet(); } catch(e) {}
      }

      // 必須シート（使い方・説明書・サポート・メモ）が無ければ①からコピー
      ['使い方', '説明書', 'サポート', 'メモ'].forEach(function(n) {
        if (clientSs.getSheetByName(n)) return;
        try {
          var _src = masterSs.getSheetByName(n);
          if (_src) { _src.copyTo(clientSs).setName(n); } else { clientSs.insertSheet(n); }
        } catch(e) {}
      });
      ...
      if (clientScriptId) {
        ...
        resultLines.push('❌ ' + companyName + '\n   スクリプトID未登録（K列が空）');
        errorNames.push(companyName + '（スクリプトID未登録）');
      }
...
function repairOneClientSS() {
  ...
  var match = input.match(/\/d\/([a-zA-Z0-9_-]+)/) || input.match(/^([a-zA-Z0-9_-]{20,})$/);
  if (!match) { ui.alert('URLまたはIDの形式が正しくありません。'); return; }
  var clientSsId = match[1];

  var props = PropertiesService.getScriptProperties();
  var approvedVersion = props.getProperty('approvedLibVersion') || props.getProperty('lastLibVersionNum') || '';
  if (!approvedVersion) { ui.alert('approvedLibVersionが未設定です。先に「📡 テスト客SS②に反映」を実行してください。'); return; }
  ...
  try {
    ...
    var allScriptIds = ssScriptMap[clientSsId] || [];
    var webAppUrl = '';
    // 会社登録シートからWebアプリURLを探す
    var masterSs = SpreadsheetApp.getActiveSpreadsheet();
    var regSheet = masterSs.getSheetByName('会社登録');
    if (regSheet) {
      var rows = regSheet.getRange(2, 1, Math.max(regSheet.getLastRow()-1,1), 13).getValues();
      for (var i = 0; i < rows.length; i++) {
        var url = String(rows[i][5] || '');
        if (url.indexOf(clientSsId) >= 0) { webAppUrl = String(rows[i][6] || ''); break; }
      }
    }
    var primaryId = '';
    if (allScriptIds.length === 1) {
      ...
      primaryId = findWebAppScriptId_(allScriptIds, webAppUrl) || allScriptIds[0];
    }
    if (!primaryId) { ui.alert('スクリプトIDが特定できませんでした。会社登録シートのURLを確認してください。'); return; }

    // 全スクリプトを一度すべてスタブで上書き（古い幽霊も含め強制更新）
    var updated = 0, failed = [];
    for (var j = 0; j < allScriptIds.length; j++) {
      var r = updateStubVersion_(allScriptIds[j], approvedVersion, false);
      if (r && r.ok) { updated++; } else {
        // primaryでなければ無効化
        if (allScriptIds[j] !== primaryId) { neutralizeScript_(allScriptIds[j]); }
...
// ================================================================
//  14-3d: コンテナバインドスクリプトを削除（deleteScriptProject_）
//  Drive API DELETE で幽霊スクリプトを完全削除する。
//  削除不可（403等）の場合は false を返す → 呼び出し元で neutralizeScript_ にフォールバック。
// ================================================================
function deleteScriptProject_(scriptId) {
  try {
    var token = ScriptApp.getOAuthToken();
    var resp = UrlFetchApp.fetch(
      'https://www.googleapis.com/drive/v3/files/' + scriptId + '?supportsAllDrives=true',
...
//             貼付シートの1行目上に行を挿入し辞書照合結果を書き込む
//             VALID_LABELSは種別（unkou/master/cust）ごとに切り替え
//             辞書はtype='common'のエントリも全種別に適用する
//             マッチングは「完全一致 or 先頭一致かつ残余が数字・括弧のみ」に限定（住所コード→住所の誤マッチを防止）
//             辞書に一致しない列はデータ値パターン（電話番号形式/郵便番号形式/カタカナ）で推定するフォールバックあり
//             cust VALID_LABELS: 取引先名カナ/入金サイクル/入金サイト/受領書送付先郵便番号/受領書送付先住所を追加
//   13-4b : getPasteImportRows(companySsId)
//             貼付シートのデータをすべて2次元配列で返す（HTML→GAS API）
//   13-4c : decomposeTonCarType_(fmap)
//             fieldMapのトン数・車種の複合値（4W, 13w, 2t平等）を自動分解する
//   13-4d : doImportFromSheet_(ss, sh)
//             PASTE_IMPORT_TYPE で取込先（運行/自車専属マスタ/取引先）を決定し一括反映しシートを削除する
//             masterは車番・乗務員名が両方空の行をスキップ（無効レコード除外）
//   13-4e : upsertMasterSheets_(ss, mappedRows)
//             運行取込データで自車専属マスタ・マスタ（取引先）をupsertする内部補助
...
//             type='common'のエントリはすべての種別で返す。辞書未作成なら自動生成
//   13-6  : importBulkRows(sheetType, mappedRows, companySsId)
//             マッピング済み行データを対象シートに一括登録する（HTML→GAS API）
//             IDはすべてシステム採番（V-/S-/M-）、LockServiceで排他制御
//             【master】車番＋乗務員名が両方一致→既存行と照合。
//               会社名〜携帯番号（D〜J列）が全一致→スキップ、1項目でも違えば上書き更新、不一致→新規追加
//               同一バッチ内の同組み合わせは2件目以降スキップ
//             【cust】列数不足時のみinsertColumnsAfterで補完（移行はinitImportDictionary_が担う）
...
//             貼付シートrow2でユーザーが「-」→フィールド名に変更した際に辞書エイリアスを追加する
//             既にエイリアスとして登録済みなら無視（重複追加しない）
...
//             手動マッピングで使われたExcel列名を辞書に自動追記する（HTML→GAS API）
...
function createImportTestSheetUnkou() {
  ...
  // ヘッダー: 辞書エイリアス表記。区分は専属/自車/空白のみ。有休・休みは積地に記入する運用
  var headers = ['配車日','看板','車種','ナンバー','運転手','携帯番号','集荷先','配送先','運賃','高速（請求）','高速（実費）','支払額','荷主','区分'];
...
function createPasteImportSheet_(sheetType) {
  ...
  sh.setTabColor(colorMap[sheetType] || '#f57f17');
  ss.setActiveSheet(sh);
  SpreadsheetApp.getUi().alert('「' + shName + '」シートを作成しました（取込先: ' + labelMap[sheetType] + '）。\n取り込みたいデータ（ヘッダー行含む）をそのまま1行目から貼り付けてください。\n貼り付け後、メニューから「▶ 貼付データを取込」を実行してください。');
}
...
function executePasteImport() {
  ...
  if (!sh) {
    SpreadsheetApp.getUi().alert('「' + _shName + '」シートが見つかりません。\n先に「📋 貼付シート作成」を実行してください。');
    return;
  }
  var mappingDone = sh.getRange(2, 1).getBackground() === '#e3f2fd';
  if (!mappingDone) {
    applyPasteImportMapping_(ss, sh);
  }
  // マッピング行（行1）を読んでトン数/車種の分割処理（▶1回目・2回目共通）
  var lastColS = Math.max(sh.getLastColumn(), 1);
  var lastRowS = sh.getLastRow();
  var mapRowS  = sh.getRange(2, 1, 1, lastColS).getValues()[0];
  var tonColS  = -1, carColS = -1;
  mapRowS.forEach(function(v, ci) {
    var lbl = String(v || '').trim();
    if (lbl === 'トン数') tonColS = ci;
  ...
  if ((tonColS >= 0 || carColS >= 0) && lastRowS >= 3) {
    var chkCols = [];
    if (tonColS >= 0) chkCols.push(tonColS);
    if (carColS >= 0) chkCols.push(carColS);
    chkCols.forEach(function(chkIdx) {
      sh.getRange(3, chkIdx + 1, lastRowS - 2, 1).getDisplayValues().forEach(function(r) {
        var raw = normZen_(r[0].trim());
  ...
  if (doSplit) {
    if (tonColS >= 0 && carColS < 0) {
      // パターンA：トン数列の右に車種列挿入、t/トン除去も全行実施
      sh.insertColumnAfter(tonColS + 1);
      var newColA  = sh.getLastColumn();
      var datAr    = sh.getRange(3, 1, lastRowS - 2, newColA).getValues();
      ...
      sh.getRange(2, 1, 1, mapRowS.length).setValues([mapRowS]);
    } else if (carColS >= 0 && tonColS < 0) {
      // パターンB：車種列の左にトン数列挿入、t/トン除去も全行実施
      sh.insertColumnBefore(carColS + 1);
      var newColB  = sh.getLastColumn();
      var datBr    = sh.getRange(3, 1, lastRowS - 2, newColB).getValues();
      ...
      sh.getRange(2, 1, 1, mapRowS.length).setValues([mapRowS]);
    } else if (tonColS >= 0 && carColS >= 0) {
      // パターンC：列挿入なし、displayValues使用でt/トン除去＋車種分割
      var datCr  = sh.getRange(3, 1, lastRowS - 2, lastColS).getValues();
      var dispCT = sh.getRange(3, tonColS + 1, lastRowS - 2, 1).getDisplayValues();
      var dispCC = sh.getRange(3, carColS + 1, lastRowS - 2, 1).getDisplayValues();
      var chgC   = false;
      datCr.forEach(function(row, ri) {
        var tv = normZen_(dispCT[ri][0].trim()), cv = normZen_(dispCC[ri][0].trim());
        var mT = tv.match(/^(\d+(?:\.\d+)?)(?:t|トン)?([\s\S]*)$/i);
        if (mT && mT[1]) {
          row[tonColS] = mT[1];
          if (mT[2].trim() && !cv) row[carColS] = mT[2].trim();
          chgC = true;
        }
      ...
      if (chgC) sh.getRange(3, 1, datCr.length, lastColS).setValues(datCr);
    }
  }
  // t/トンのみで車種なし → 列挿入なしでトン数除去のみ実施
  if (!doSplit && doTonClean && tonColS >= 0) {
    var datTC  = sh.getRange(3, 1, lastRowS - 2, lastColS).getValues();
    var dispTC = sh.getRange(3, tonColS + 1, lastRowS - 2, 1).getDisplayValues();
    var chgTC  = false;
    datTC.forEach(function(row, ri) {
      var raw = dispTC[ri][0].trim();
      var m = raw.match(/^(\d+(?:\.\d+)?)(?:t|トン)?$/i);
  ...
  SpreadsheetApp.getUi().alert('マッピング済みです。\n「✅ この内容で確定・反映」を押して取込を実行してください。');
}
...
// ================================================================
function applyPasteImportMapping_(ss, sh) {
  sh.getProtections(SpreadsheetApp.ProtectionType.SHEET).forEach(function(p){p.remove();});
  sh.getProtections(SpreadsheetApp.ProtectionType.RANGE).forEach(function(p){p.remove();});
  ...
  if (setting && setting.getLastRow() >= 2) {
    ...
    rows.forEach(function(r) {
      var _rType = String(r[0] || '').trim();
      if (_rType === pType || _rType === 'common') {
  ...
  var resultRow = hdrs.map(function(h, colIdx) {
    ...
    var _smp = dataVals.slice(0, 5).map(function(row) { return String(row[colIdx] || '').trim(); }).filter(Boolean);
    if (_smp.length > 0) {
  ...
  // 2行目に1行挿入してマッピング行を配置（1行目の元ヘッダーはそのまま残す）
  sh.insertRowBefore(2);
  var totalCols = resultRow.length;
  sh.getRange(2, 1, 1, totalCols).setValues([resultRow]);
  // プルダウン設定（種別ごとに使用可能なフィールド名を制限）
  var VALID_LABELS = pType === 'master'
  ...
  var rule = SpreadsheetApp.newDataValidation()
    ...
    .setAllowInvalid(false)
    .build();
  sh.getRange(2, 1, 1, totalCols).setDataValidation(rule);
  sh.getRange(2, 1, 1, totalCols)
    .setBackground('#e3f2fd').setFontWeight('bold')
    .setNote('プルダウンで項目名を選択してください。不要な列は「-」を選んでください。確認後「✅ この内容で確定・反映」を押してください。');
}
...
function getPasteImportRows(companySsId) {
  ...
  var lastCol = Math.max(sh.getLastColumn(), 1);
  return sh.getRange(1, 1, lastRow, lastCol).getValues().map(function(row) {
...
// ================================================================
//  13-4c: トン数・車種自動分解（decomposeTonCarType_）  【大B / 中13 / 小13-4c】
//  fieldMapのton/carTypeに「4W」「13w」「2t平」等の複合値がある場合に分解する
//  数字部分→ton, t/T除く文字部分→carType
// ================================================================
function decomposeTonCarType_(fmap) {
  ...
  if (!m || !m[1]) {
    // 数字で始まらない値（W・平・箱等）がトン数フィールドにある場合は車種に移す
    if (tonVal && !carTypeVal) {
      fmap['carType'] = tonVal;
      fmap['ton']     = '';
    }
...
function doImportFromSheet_(ss, sh) {
  ...
  try {
    var dispToFid = {
      // 運行
      '日付':'date',     '車番':'car',        '乗務員名':'driver',    '区分':'division',
      '会社名':'company','トン数':'ton',       '車種':'carType',      '携帯番号':'phone',
      '看板名':'signboard','荷主':'client',   '積地':'pickPlace',    '降地':'dropPlace',
      ...
      '高速を引く':'tollDeduct',
      '車両リース代':'leaseCar',     '任意保険料':'insVoluntary',
      '自賠責保険料':'insCompulsory','重量税積立':'taxWeight',
      '車検費積立':'inspectionFund','整備費積立':'maintenanceFund',
      'タイヤ代積立':'tireFund',    '修理積立':'repairFund',
      '駐車場代':'parking',         'ETCリース料':'etcLease',
      'カーナビリース料':'naviLease','通信費':'commFee',
      '洗車費':'carWash',            '制服費':'uniform',    'その他固定費':'otherFixed',
      '免許証有効期限':'licenseExpiry','安全教育次回予定日':'safetyEduDate',
      '健康診断次回予定日':'healthCheckDate','適性診断次回予定日':'aptTestDate',
      '担当管理者':'managerName',
      // 取引先（マスタ）
      '電話':'tel',      'FAX':'fax',         '郵便番号':'zip',      '住所':'address',
      '代表者':'rep',    '配車担当':'contact',
      '銀行名':'bank',   '支店名':'branch',   '種別':'bankType',
      '番号':'accountNo','名義':'accountName','メールアドレス':'email',
      '取引先名カナ':'kana', '入金サイクル':'paymentCycle', '入金サイト':'paymentSite',
      '受領書送付先郵便番号':'receiptZip', '受領書送付先住所':'receiptAddress',
      'インボイス登録番号':'invoiceNo',  'インボイス発行者名（自社名）':'invoiceName'
    };
    var lastCol = Math.max(sh.getLastColumn(), 1);
    var lastRow = sh.getLastRow();
    if (lastRow < 3) {
      ui.alert('データ行がありません。ヘッダー行を含むデータを貼り付けてください。');
      return;
    }
    var mapRow  = sh.getRange(2, 1, 1, lastCol).getValues()[0];
    var fidCols = mapRow.map(function(v) {
      var key = String(v || '').trim();
      return (key === '-' || !key) ? null : (dispToFid[key] || null);
    });
    ...
    dataVals.forEach(function(row, _ri) {
      ...
      if (!String(fmap['ton'] || '').trim() && !String(fmap['carType'] || '').trim()) {
        fidCols.forEach(function(fid, ci) {
          if (fid !== null) return;
          var d = String(row[ci] || '').trim();
          if (!d) return;
          var m = d.match(/^(\d+(?:\.\d+)?)\s*(t|T|トン|[^\d\s\r\n])([\s\S]*)$/i);
          if (!m) return;
          if (!String(fmap['ton'] || '').trim()) {
            fmap['ton'] = m[1];
            var rest = (/^(t|T|トン)$/i.test(m[2]) ? '' : m[2]) + m[3];
            if (rest.trim() && !String(fmap['carType'] || '').trim()) fmap['carType'] = rest.trim();
          }
        });
      }
      decomposeTonCarType_(fmap);
      // master: 車番・乗務員名がどちらも空の行はスキップ（無効レコード）
      if (_ptCheck === 'master' && !String(fmap['car'] || '').trim() && !String(fmap['driver'] || '').trim()) return;
...
// ================================================================
//  13-4e: マスタupsert（upsertMasterSheets_）  【大B / 中13 / 小13-4e】
//  運行取込データで自車専属マスタ・マスタ（取引先）をupsertする内部補助
// ================================================================
function upsertMasterSheets_(ss, mappedRows) {
  // --- 自車専属マスタ ---
  var jisha = ss.getSheetByName('自車専属マスタ');
  if (jisha) {
    var jishaKeys = {}, jishaList = [];
    mappedRows.forEach(function(fmap) {
      var car    = String(fmap['car']    || '').trim();
      var driver = String(fmap['driver'] || '').trim();
      if (!car && !driver) return;
      var key = car || driver;
      if (!jishaKeys[key]) { jishaKeys[key] = true; jishaList.push(fmap); }
    });
    if (jishaList.length > 0) {
      var jlr   = jisha.getLastRow();
      var jCols = Math.max(jisha.getLastColumn(), 13);
      var jData = jlr >= 2 ? jisha.getRange(2, 1, jlr - 1, jCols).getValues() : [];
      // 更新対象フィールドと列番号（0-indexed）
      var fidColIdx = { division:2, company:3, signboard:4, ton:5, carType:6, car:7, driver:8, phone:9 };
      var jToAdd = [];
      jishaList.forEach(function(fmap) {
        var car    = String(fmap['car']    || '').trim();
        var driver = String(fmap['driver'] || '').trim();
        var found  = -1;
        for (var i = 0; i < jData.length; i++) {
          var mCar    = String(jData[i][7] || '').trim(); // col 8: 車番
          var mDriver = String(jData[i][8] || '').trim(); // col 9: 乗務員名
          if (car && mCar === car) { found = i; break; }
          if (!car && driver && mDriver === driver) { found = i; break; }
        }
        if (found >= 0) {
          var row = jData[found].slice();
          Object.keys(fidColIdx).forEach(function(fid) {
            var val = String(fmap[fid] || '').trim();
            if (val) row[fidColIdx[fid]] = val;
          });
          jisha.getRange(found + 2, 1, 1, row.length).setValues([row]);
        } else {
          jToAdd.push(fmap);
        }
      });
      if (jToAdd.length > 0) {
        var jStart  = Math.max(jisha.getLastRow() + 1, 2);
        var jNextId = getNextIdNum_(jisha, 'S');
        var jRows   = jToAdd.map(function(fmap, i) {
          return buildSheetRow_('master', 'S-' + String(jNextId + i).padStart(4, '0'), fmap, ss);
        });
        commitLastId_(jisha, 'S', jNextId + jToAdd.length - 1);
        jisha.getRange(jStart, 1, jRows.length, jRows[0].length).setValues(jRows);
      }
    }
  }

  // --- マスタ（取引先） ---
  var cust = ss.getSheetByName('マスタ');
  if (cust) {
    // 荷主（client）・会社名（company）の両方を照合対象にする
    var custKeys = {}, custList = [];
    mappedRows.forEach(function(fmap) {
      ['client', 'company'].forEach(function(fid) {
        var name = String(fmap[fid] || '').trim();
        if (!name || custKeys[name]) return;
        custKeys[name] = true;
        custList.push(name);
      });
    });
    if (custList.length > 0) {
      var clr   = cust.getLastRow();
      var cData = clr >= 2 ? cust.getRange(2, 1, clr - 1, 2).getValues() : [];
      var cToAdd = custList.filter(function(name) {
        return !cData.some(function(r) { return String(r[1] || '').trim() === name; });
      });
      if (cToAdd.length > 0) {
        var cStart  = Math.max(cust.getLastRow() + 1, 2);
        var cNextId = getNextIdNum_(cust, 'M');
        var cRows   = cToAdd.map(function(name, i) {
          return buildSheetRow_('cust', 'M-' + String(cNextId + i).padStart(4, '0'), { company: name }, ss);
        });
        commitLastId_(cust, 'M', cNextId + cToAdd.length - 1);
        // initImportDictionary_でレイアウト移行済みのはずだが念のため列数を確保
        if (cust.getMaxColumns() < cRows[0].length) {
          cust.insertColumnsAfter(cust.getMaxColumns(), cRows[0].length - cust.getMaxColumns());
        }
        cust.getRange(cStart, 1, cRows.length, cRows[0].length).setValues(cRows);
      }
    }
  }
}
...
function confirmPasteImport() {
  ...
  if (sh.getRange(2, 1).getBackground() !== '#e3f2fd') {
    SpreadsheetApp.getUi().alert('先に「▶ 貼付データを取込」を実行してマッピング内容を確認してください。');
    return;
  }
  doImportFromSheet_(ss, sh);
}
...
function getImportDictionary(sheetType, companySsId) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (type !== sheetType && type !== 'common') continue;
...
function importBulkRows(sheetType, mappedRows, companySsId, isLastChunk, allPaymentRows) {
  ...
  try {
    // ── master: upsert（車番/乗務員名で既存行照合→更新 or 新規追加）＋燃費自動補完 ──
    if (sheetType === 'master') {
      var _mFuelMap = {};
      var _mSetting = ss.getSheetByName('設定');
      if (_mSetting && _mSetting.getLastRow() >= 2) {
        _mSetting.getRange(2, 1, _mSetting.getLastRow() - 1, 2).getValues().forEach(function(r) {
          var k = String(r[0] || '').trim();
          if (k && r[1] !== '' && r[1] !== null) _mFuelMap[k] = r[1];
        });
      }
      var _mLastR = sheet.getLastRow();
      var _mExist = _mLastR >= 2 ? sheet.getRange(2, 1, _mLastR - 1, Math.max(sheet.getLastColumn(), 13)).getValues() : [];
      // 照合キー: 車番+"|"+乗務員名 が両方一致した行のみupsert対象
      var _mKeyMap = {};
      for (var _mi = 0; _mi < _mExist.length; _mi++) {
        var _mExCar = String(_mExist[_mi][7] || '').trim();
        var _mExDrv = String(_mExist[_mi][8] || '').trim();
        if (_mExCar && _mExDrv) _mKeyMap[_mExCar + '|' + _mExDrv] = _mi + 2;
      }
      var _mNextNum = getNextIdNum_(sheet, prefix);
      var _mInsertRows = [], _mInsertCount = 0, _mBatchKeys = {};
      for (var _mj = 0; _mj < mappedRows.length; _mj++) {
        var _mFm  = mappedRows[_mj];
        var _mCar = String(_mFm['car']    || '').trim();
        var _mDrv = String(_mFm['driver'] || '').trim();
        var _mKey = _mCar + '|' + _mDrv;
        // 車番・乗務員名が両方一致する場合のみ同一レコードとみなす
        var _mFoundRow = (_mCar && _mDrv) ? (_mKeyMap[_mKey] || 0) : 0;
        // 同バッチ内で既に処理済みの組み合わせはスキップ
        if (_mCar && _mDrv && _mBatchKeys[_mKey]) continue;
        var _mUseId;
        if (_mFoundRow) {
          _mUseId = String(_mExist[_mFoundRow - 2][0] || '').trim();
          if (!_mUseId) { _mUseId = prefix + '-' + String(_mNextNum + _mInsertCount).padStart(4, '0'); _mInsertCount++; _mFoundRow = 0; }
        } else {
          _mUseId = prefix + '-' + String(_mNextNum + _mInsertCount).padStart(4, '0');
          _mInsertCount++;
        }
        var _mRow = buildSheetRow_('master', _mUseId, _mFm, ss);
        // 燃費が空でトン数あり→設定シートから自動補完
        if ((_mRow[11] === '' || _mRow[11] === null || _mRow[11] === 0) && String(_mRow[5] || '').trim()) {
          var _mTK = String(_mRow[5]).trim();
          if (_mFuelMap[_mTK] !== undefined) _mRow[11] = _mFuelMap[_mTK];
        }
        if (_mFoundRow) {
          // 会社名(idx3)〜携帯番号(idx9)が全て一致→完全重複のためスキップ
          var _mExRow = _mExist[_mFoundRow - 2];
          var _mAllSame = true;
          for (var _mc = 3; _mc <= 9; _mc++) {
            if (String(_mRow[_mc] || '').trim() !== String(_mExRow[_mc] || '').trim()) { _mAllSame = false; break; }
          }
          if (!_mAllSame) sheet.getRange(_mFoundRow, 1, 1, _mRow.length).setValues([_mRow]);
        } else {
          _mInsertRows.push(_mRow);
        }
        if (_mCar && _mDrv) _mBatchKeys[_mKey] = true; // 処理済みフラグ
      }
      if (_mInsertRows.length > 0) {
        commitLastId_(sheet, prefix, _mNextNum + _mInsertRows.length - 1);
        var _mStart = Math.max(sheet.getLastRow() + 1, 2);
        sheet.getRange(_mStart, 1, _mInsertRows.length, _mInsertRows[0].length).setValues(_mInsertRows);
      }
    ...
    if (sheetType === 'unkou') {
      ...
      if (isLastChunk !== false) {
        ...
        sortUnkouByDate_(companySsId, true);
...
// ================================================================
//  13-7: シート行データ構築（buildSheetRow_）  【大B / 中13 / 小13-7】
//  fieldIdとその値のマップを受け取り各シートの列順に合わせた配列を返す
//  運行シートは車番または乗務員名で自車専属マスタを検索し区分・会社名等を補完する
//  cust（22列）: 1=ID, 2=取引先名カナ, 3=会社名, 4=電話, 5=FAX, 6=郵便番号, 7=住所,
//               8=代表者, 9=配車担当, 10-14=銀行等, 15=備考, 16=メールアドレス,
//               17=入金サイクル, 18=入金サイト, 19=受領書送付先郵便番号, 20=受領書送付先住所,
//               21=インボイス登録番号, 22=インボイス発行者名
// ================================================================
function buildSheetRow_(sheetType, id, fieldMap, ss) {
  ...
  if (sheetType === 'unkou') {
    ...
    return [
      ...
      company  || String(fieldMap['company']  || '') || String(fieldMap['signboard'] || ''),  //  3: 会社名（空なら看板名をデフォルト）
      normalizeTons_(String(fieldMap['ton']     || '') || ton),   //  4: トン数
      (String(fieldMap['carType'] || '') || carType).replace(/[Ａ-Ｚａ-ｚ]/g, function(c){return String.fromCharCode(c.charCodeAt(0)-0xFEE0);}).toUpperCase(), //  5: 車種
      car,                                                 //  6: 車番
      driver,                                              //  7: 乗務員名
      ...
      String(fieldMap['timeGuide']      || ''),             // 14: 誘導時刻
      String(fieldMap['timePick']       || ''),             // 15: 積完時刻
      String(fieldMap['timeBreakStart'] || ''),             // 16: 休憩開始
      String(fieldMap['timeBreakEnd']   || ''),             // 17: 休憩終了
      String(fieldMap['timeDrop']       || ''),             // 18: 降完時刻
  ...
  if (sheetType === 'master') {
    ...
    return [
      ...
      normalizeTons_(String(fieldMap['ton'] || '')),       //  6: トン数（正規化）
      (String(fieldMap['carType'] || '')).replace(/[Ａ-Ｚａ-ｚ]/g, function(c){return String.fromCharCode(c.charCodeAt(0)-0xFEE0);}).toUpperCase(), //  7: 車種（半角大文字）
      String(fieldMap['car']       || ''),                 //  8: 車番
      String(fieldMap['driver']    || ''),                 //  9: 乗務員名
      String(fieldMap['phone']     || ''),                 // 10: 携帯番号
      String(fieldMap['email']     || ''),                 // 11: アドレス
      toImportNum_(fieldMap['fuel']),                      // 12: 燃費（数値化）
      String(fieldMap['memo']              || ''),          // 13: 備考
      toImportNum_(fieldMap['provisionalDays']),            // 14: 仮日数
      toImportNum_(fieldMap['salary']),                     // 15: 給料
      toImportNum_(fieldMap['salaryPct']),                  // 16: ％
      String(fieldMap['tollDeduct']        || ''),          // 17: 高速を引く
      toImportNum_(fieldMap['leaseCar']),                   // 18: 車両リース代
      toImportNum_(fieldMap['insVoluntary']),               // 19: 任意保険料
      toImportNum_(fieldMap['insCompulsory']),              // 20: 自賠責保険料
      toImportNum_(fieldMap['taxWeight']),                  // 21: 重量税積立
      toImportNum_(fieldMap['inspectionFund']),             // 22: 車検費積立
      toImportNum_(fieldMap['maintenanceFund']),            // 23: 整備費積立
      toImportNum_(fieldMap['tireFund']),                   // 24: タイヤ代積立
      toImportNum_(fieldMap['repairFund']),                 // 25: 修理積立
      toImportNum_(fieldMap['parking']),                    // 26: 駐車場代
      toImportNum_(fieldMap['etcLease']),                   // 27: ETCリース料
      toImportNum_(fieldMap['naviLease']),                  // 28: カーナビリース料
      toImportNum_(fieldMap['commFee']),                    // 29: 通信費
      toImportNum_(fieldMap['carWash']),                    // 30: 洗車費
      toImportNum_(fieldMap['uniform']),                    // 31: 制服費
      toImportNum_(fieldMap['otherFixed']),                 // 32: その他固定費
      '',                                                   // 33: PL設定按分（参照・数式）
      String(fieldMap['licenseExpiry']     || ''),          // 34: 免許証有効期限
      String(fieldMap['safetyEduDate']     || ''),          // 35: 安全教育次回予定日
      String(fieldMap['healthCheckDate']   || ''),          // 36: 健康診断次回予定日
      String(fieldMap['aptTestDate']       || ''),          // 37: 適性診断次回予定日
      String(fieldMap['managerName']       || '')
    ];
  }

  if (sheetType === 'cust') {
    // マスタ（取引先）22列（custHeader参照）
    return [
      id,                                                  //  1: マスタID
      String(fieldMap['kana']          || ''),             //  2: 取引先名カナ
      String(fieldMap['company'] || ''),                   //  3: 会社名
      String(fieldMap['tel']     || ''),                   //  4: 電話
      String(fieldMap['fax']     || ''),                   //  5: FAX
      String(fieldMap['zip']     || ''),                   //  6: 郵便番号
      String(fieldMap['address'] || ''),                   //  7: 住所
      String(fieldMap['rep']     || ''),                   //  8: 代表者
      String(fieldMap['contact']       || ''),              //  9: 配車担当
      String(fieldMap['bank']          || ''),              // 10: 銀行名
      String(fieldMap['branch']        || ''),              // 11: 支店名
      String(fieldMap['bankType']      || ''),              // 12: 種別
      String(fieldMap['accountNo']     || ''),              // 13: 番号
      String(fieldMap['accountName']   || ''),              // 14: 名義
      String(fieldMap['memo']          || ''),              // 15: 備考
      String(fieldMap['email']         || ''),              // 16: メールアドレス
      String(fieldMap['paymentCycle']  || ''),              // 17: 入金サイクル
      String(fieldMap['paymentSite']   || ''),              // 18: 入金サイト
      String(fieldMap['receiptZip']    || ''),              // 19: 受領書送付先郵便番号
      String(fieldMap['receiptAddress']|| ''),              // 20: 受領書送付先住所
      String(fieldMap['invoiceNo']     || ''),              // 21: インボイス登録番号
      String(fieldMap['invoiceName']   || '')
    ];
  }
  return [id];
}
...
function initImportDictionary_(ss) {
  ...
  // 取引先シート（マスタ）の列構造を正規22列レイアウトへ自動移行
  ...
  var defaults = [
    ...
    ['unkou', 'payment',        '支払い',        '支払い,支払,支払額,給与,手取り,運転手支払,外注費,外注,傭車費,傭車代,協力会社費,下請代,業者支払,協力費,委託費,ドライバー支払,委託運賃,外注運賃,支払金額'],
    ['unkou', 'timeGuide',      '誘導時刻',      '誘導時刻,誘導,誘導時間,誘導開始,出発時刻,出発時間,出庫時刻,出庫時間,出発,乗務開始'],
    ['unkou', 'timePick',       '積完時刻',      '積完時刻,積完,積完時間,積込完了,積込完了時刻,積完了,荷積完了,積込時刻,積地到着,積込時間'],
    ...
    ['master', 'carType',  '車種',          '車種,形状,ボディ,車体形状,車両形状,ボディ形状,車体'],
    ['master', 'car',      '車番',          '車番,登録番号,ナンバー,車両番号,プレート,号車,車輌番号,車輌,ナンバープレート,車No'],
    ['master', 'driver',   '乗務員名',      '乗務員名,氏名,ドライバー,運転手,担当者,乗務員,担当ドライバー,ドライバー名,運転手名'],
    ['master', 'phone',    '携帯番号',      '携帯番号,連絡先,電話番号,スマホ,TEL,携帯,携帯電話,携帯Tel,携帯tel,連絡電話'],
    ['master', 'signboard','看板名',        '看板名,看板,荷主看板,荷主看板名,看板会社名,表示名称,表示会社名,荷主表示名,屋号,通称名,ブランド名,取引先略称,荷主略称'],
    ['master', 'email',           'メールアドレス','メール,メールアドレス,mail,email,アドレス,e-mail,MailAddress'],
    ['master', 'fuel',            '燃費',          '燃費,燃料消費率,燃費(km/L),油費,燃費率'],
    ['master', 'provisionalDays', '仮日数',        '仮日数,稼働日数,出勤日数,乗務日数,予定日数,乗務予定日数'],
    ['master', 'salary',          '給料',          '給料,給与,月給,基本給,賃金,報酬,ドライバー給与,乗務員給与'],
    ['master', 'salaryPct',       '給料割合',      '％,%,割合,パーセント,比率,給与割合,配分率,歩合率,給料比率'],
    ['master', 'tollDeduct',      '高速を引く',    '高速を引く,高速控除,高速差引,高速引く,高速減額'],
    ['master', 'leaseCar',        '車両リース代',  '車両リース代,リース代,リース料,車両リース料,車輌リース,車両費,リース費用'],
    ['master', 'insVoluntary',    '任意保険料',    '任意保険料,任意保険,車両保険,対人対物保険,任意保険費,保険料（任意）'],
    ['master', 'insCompulsory',   '自賠責保険料',  '自賠責保険料,自賠責,自賠責保険,強制保険,自賠責費,保険料（自賠責）'],
    ['master', 'taxWeight',       '重量税積立',    '重量税積立,重量税,車両重量税,重量税積み立て,重量税費'],
    ['master', 'inspectionFund',  '車検費積立',    '車検費積立,車検費,車検代,車検積立,車検費用積立,車検費積み立て'],
    ['master', 'maintenanceFund', '整備費積立',    '整備費積立,整備費,整備代,整備積立,メンテ費,整備費用積立,整備費積み立て'],
    ['master', 'tireFund',        'タイヤ代積立',  'タイヤ代積立,タイヤ代,タイヤ費,タイヤ積立,タイヤ費用積立,タイヤ代積み立て'],
    ['master', 'repairFund',      '修理積立',      '修理積立,修理費,修理代,修理積み立て,修繕費,修繕積立,修理費用積立'],
    ['master', 'parking',         '駐車場代',      '駐車場代,駐車場,駐車料金,駐車費,駐車代,パーキング代,駐車場費用'],
    ['master', 'etcLease',        'ETCリース料',   'ETCリース料,ETC料金,ETC,ETCリース,ETC費用,ETC代'],
    ['master', 'naviLease',       'カーナビリース料','カーナビリース料,カーナビ,ナビリース,カーナビ費用,ナビ代,カーナビ代'],
    ['master', 'commFee',         '通信費',        '通信費,通信料,電話代,スマホ代,携帯料金,通信費用,通話料'],
    ['master', 'carWash',         '洗車費',        '洗車費,洗車代,洗車料,洗車費用,クリーニング代'],
    ['master', 'uniform',         '制服費',        '制服費,制服代,ユニフォーム代,作業服代,制服費用,被服費'],
    ['master', 'otherFixed',      'その他固定費',  'その他固定費,その他費用,雑費,その他,雑経費,その他経費,固定費（その他）'],
    ['master', 'licenseExpiry',   '免許証有効期限','免許証有効期限,免許有効期限,免許期限,免許証期限,免許証有効日,有効期限（免許）'],
    ['master', 'safetyEduDate',   '安全教育次回予定日','安全教育次回予定日,安全教育,次回安全教育,安全教育予定,安全教育日'],
    ['master', 'healthCheckDate', '健康診断次回予定日','健康診断次回予定日,健康診断,次回健康診断,健康診断予定,健診予定,健診日'],
    ['master', 'aptTestDate',     '適性診断次回予定日','適性診断次回予定日,適性診断,次回適性診断,適性診断予定,適性検査,適性検査日'],
    ['master', 'managerName',     '担当管理者',    '担当管理者,管理者,管理担当,担当者（管理）,担当管理,管理担当者,監督者'],
    // 取引先（マスタ）固有
    ['cust', 'tel',           '電話',                '電話,TEL,電話番号,代表電話,電話(代表),Tel,phone,連絡電話,取引先TEL,取引先電話,取引先Tel,主要電話,Tel番号,電話(代表電話),代表Tel'],
    ['cust', 'fax',           'FAX',                 'FAX,ファックス,FAX番号,Fax,fax番号,取引先FAX,取引先Fax,FAX（代表）,ファクシミリ'],
    ['cust', 'zip',           '郵便番号',            '郵便番号,〒,zip,ZIP,郵便,Post,郵便No'],
    ['cust', 'address',       '住所',                '住所,所在地,住所1,住所2,本社住所,所在,本社,住所（全体）,所在地（都道府県）,本社所在地,所在地住所,住所全体,番地,所在地全体'],
    ['cust', 'rep',           '代表者',              '代表者,代表,社長,責任者,代表取締役,代表者名,代表者氏名,代表取締役名,代表取締役氏名,法人代表,代表名'],
    // ※ 帝国DB評点は取引先データによく含まれる信用評点欄。配車担当とは別物なので含めない
    ['cust', 'contact',       '配車担当',            '配車担当,担当者,担当,配車係,配車担当者,担当者名,担当者氏名,連絡担当,連絡担当者,ご担当,ご担当者,担当窓口,担当営業,配送担当,集荷担当,担当配車,窓口担当,受付担当,担当者連絡先,担当者名前,主担当,担当スタッフ'],
    // ※ 取引先名略称は会社略称（会社名と同一）であり、カナ名ではないため含めない
    ['cust', 'kana',          '取引先名カナ',        '取引先名カナ,カナ,カナ名,読み,よみがな,読み仮名,フリガナ,ふりがな,取引先カナ,取引先名ふりがな,名前カナ,法人名カナ,会社名カナ,会社名（カナ）,フリガナ（取引先）'],
    ['cust', 'paymentCycle',  '入金サイクル',        '入金サイクル,サイクル,入金周期,決済サイクル,支払サイクル,支払いサイクル,回収サイクル,集金サイクル,入金頻度,締め日サイクル'],
    ['cust', 'paymentSite',   '入金サイト',          '入金サイト,入金条件,決済条件,支払条件,支払い条件,入金期日,決済期日,月末締め,支払期限,サイト,入金月,入金タイミング,回収条件,支払サイクル（日数）'],
    ['cust', 'receiptZip',    '受領書送付先郵便番号', '受領書送付先郵便番号,受領書郵便番号,受領書〒,送付先郵便番号,送付先〒,受領書送付郵便番号,書類送付先郵便番号'],
    ['cust', 'receiptAddress','受領書送付先住所',     '受領書送付先住所,受領書住所,送付先住所,受領書送付先,受領書送付地,受領書先住所,書類送付先住所,送付先'],
    ['cust', 'bank',          '銀行名',              '銀行名,銀行,金融機関名,振込先銀行,指定銀行,振込銀行,口座銀行'],
    ['cust', 'branch',        '支店名',              '支店名,支店,金融機関支店,振込先支店,支店コード,店舗名,支店（銀行）'],
    ['cust', 'bankType',      '種別',                '種別,口座種別,預金種別,口座区分,普通預金,当座預金,普通,当座,口座タイプ'],
    ['cust', 'accountNo',     '口座番号',            '番号,口座番号,口座No,口座ナンバー,振込先口座番号,账号'],
    ['cust', 'accountName',   '口座名義',            '名義,口座名義,名義人,口座名義人,振込名義,振込先名義,預金名義'],
    ['cust', 'email',         'メールアドレス',      'メールアドレス,メール,mail,email,e-mail,MailAddress,連絡メール,担当メール'],
    ['cust', 'invoiceNo',     'インボイス登録番号',  'インボイス登録番号,インボイス番号,適格請求書番号,登録番号,T番号,適格番号,インボイスNo'],
    ['cust', 'invoiceName',   'インボイス発行者名',  'インボイス発行者名（自社名）,インボイス発行者名,発行者名,インボイス名,発行者（インボイス）,適格請求書発行者名'],
    // 全種別共通（会社名・備考は運行/マスタ/取引先すべてで使用）
    // ※ 「取引先」単体を削除（取引先TELへの誤マッチ防止）。取引先名略称は会社略称として追加
    ['common', 'company',  '会社名',        '会社名,所属,協力会社,事業者名,法人名,会社,運送会社,キャリア,業者名,協力業者,配車会社,傭車先,協力会社先,外注先,下請,車両会社,荷主名,顧客名,得意先,荷主,客先,顧客,荷送人名,取引先名,取引先名略称,取引先略称,会社略称,荷主略称,法人略称'],
  ...
  setting.getRange(1, 8, 1, 4).setValues([['【辞書v10】種別', 'フィールドID', '表示名', 'エイリアス（カンマ区切り・自由に追加可）']]);
...
// ================================================================
//  13-8a: 辞書エイリアス追加（addImportDictionaryAlias_）  【大B / 中13 / 小13-8a】
// ================================================================
function addImportDictionaryAlias_(ss, sheetType, displayName, newAlias) {
  var setting = ss.getSheetByName('設定');
  if (!setting || setting.getLastRow() < 2) return;
  var data = setting.getRange(2, 8, setting.getLastRow() - 1, 4).getValues();
  for (var i = 0; i < data.length; i++) {
    if (String(data[i][0]).trim() !== sheetType) continue;
    if (String(data[i][2]).trim() !== displayName) continue;
    var aliases = String(data[i][3]).split(',').map(function(a) { return a.trim(); }).filter(Boolean);
    if (aliases.indexOf(newAlias) !== -1) return;
    aliases.push(newAlias);
    setting.getRange(i + 2, 11).setValue(aliases.join(','));
    return;
  }
}

// ================================================================
//  13-9: インポート用日付変換（parseImportDate_）  【大B / 中13 / 小13-9】
...
// ================================================================
//  13-10: インポート数値変換補助（toImportNum_）  【大B / 中13 / 小13-10】
...
// ================================================================
//  13-10a: 辞書エイリアス自動保存（saveImportAliases）  【大A / 中13 / 小13-10a】
...
function markDocumentIssued(rowId, docType, ssId) {
  ...
  for (var i = 0; i < ids.length; i++) {
    if (String(ids[i][0]||'').trim() === rowId) {
      var issuedCell = sheet.getRange(i + 2, colIdx + 1);
      issuedCell.setNumberFormat('@');
      issuedCell.setValue(Utilities.formatDate(new Date(), 'Asia/Tokyo', 'MM/dd HH:mm'));
      ss.toast(colName + ' を発行済にしました（ID: ' + rowId + '）', '✅', 3);
      return;
    }
  }
}
...
function getKyoryokuHistory(clientName, ssId) {
  ...
  try { ss = SpreadsheetApp.openById(ssId); } catch(e) { return []; }
  if (!ss) return [];
  var sh = ss.getSheetByName('協力会社履歴');
  if (!sh || sh.getLastRow() < 2) return [];
  var data = sh.getRange(2, 1, sh.getLastRow() - 1, 6).getValues();
  var results = [];
  data.forEach(function(r) {
    ...
    results.push({ signboard: String(r[1]||''), car: String(r[2]||''), driver: String(r[3]||''), phone: String(r[4]||''), lastUsed: lastUsed });
  });
  results.sort(function(a, b) { return (b.lastUsed||'').localeCompare(a.lastUsed||''); });
  return results;
}
...
function saveKyoryokuHistory(clientName, yousha, ssId) {
  ...
  try { ss = SpreadsheetApp.openById(ssId); } catch(e) { return; }
  if (!ss) return;
  saveKyoryokuHistory_(clientName, yousha, ss);
}

// ================================================================
//  14-12i: 協力会社履歴保存・内部実装（saveKyoryokuHistory_）  【大B / 中14 / 小14-12i】
//  重複チェック: 荷主名＋看板名＋車番＋乗務員名が同一なら最終使用日を更新のみ
// ================================================================
function saveKyoryokuHistory_(clientName, youshaData, ss) {
  if (!ss) return;
  var signboard = String(youshaData.signboard||'').trim();
  var car       = String(youshaData.car      ||'').trim();
  var driver    = String(youshaData.driver   ||'').trim();
  var phone     = String(youshaData.phone    ||'').trim();
  if (!signboard && !car && !driver && !phone) return;
  var sh = ss.getSheetByName('協力会社履歴');
  if (!sh) {
    sh = ss.insertSheet('協力会社履歴');
    sh.getRange(1, 1, 1, 6).setValues([['荷主名', '看板名', '車番', '乗務員名', '携帯番号', '最終使用日']]);
    sh.hideSheet();
  }
  var cName = String(clientName||'').trim();
  var now = new Date();
  if (sh.getLastRow() >= 2) {
    ...
    for (var i = 0; i < rows.length; i++) {
      if (String(rows[i][0]||'').trim() === cName &&
          String(rows[i][1]||'').trim() === signboard &&
          String(rows[i][2]||'').trim() === car &&
          String(rows[i][3]||'').trim() === driver) {
        sh.getRange(i + 2, 6).setValue(now);
        return;
      }
    }
  }
  sh.appendRow([cName, signboard, car, driver, phone, now]);
}
...
function showExportDialog() {
  ...
  var rows = sheets.map(function(s) {
    ...
    return '<tr>'
      ...
      + '<input type="checkbox" class="sheetCk" value="' + name + '" style="width:14px;height:14px;cursor:pointer;accent-color:#1565c0">'
      + '</td>'
