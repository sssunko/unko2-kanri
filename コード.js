...
//                                             getYoushaHistory(14-11), saveYoushaHistory(14-12),
//                                             saveYoushaHistory_(14-12i)
...
function createImportTestSheetUnkou() {
  ...
  var rows = [
    ...
    ['2026/08/26','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','吹田市',        '京都市伏見区',     52000,2000,1500,44000,'C物流（株）',''],
    ...
    ['2026/08/26','E食品傭車','20トレ','大阪き503','西村 幸平','090-7777-8888',  '大阪市住之江区','兵庫県西宮市',         55000,2000,2000,46000,'E食品（株）',''],
    ['2026/08/26','A運輸専属','4tW','大阪か101','水島 建二','090-1111-2222', '堺市堺区',   '京都市南区',       47000,1500,1500,40000,'A運輸（株）','専属'],
    ['2026/08/26','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','摂津市',       '奈良県生駒市',         49000,1000,1000,41000,'C物流（株）',''],
    ...
    ['2026/08/27','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','大阪市住之江区','兵庫県西宮市',         55000,2000,2000,46000,'C物流（株）',''],
    ...
    ['2026/08/27','E食品傭車','20トレ','大阪き503','西村 幸平','090-7777-8888',  '守口市',        '神戸市灘区',       38000,1000,1000,32000,'E食品（株）',''],
    ['2026/08/27','A運輸専属','4tW','大阪か101','水島 建二','090-1111-2222', '大阪市鶴見区', '滋賀県彦根市',             62000,3000,3000,52000,'A運輸（株）','専属'],
    ['2026/08/27','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','守口市',        '京都市伏見区', 48000,1500,1500,40000,'C物流（株）',''],
    ...
    ['2026/08/28','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','守口市',        '京都市伏見区', 48000,1500,1500,40000,'C物流（株）',''],
    ...
    ['2026/08/28','E食品傭車','20トレ','大阪き503','西村 幸平','090-7777-8888',  '吹田市',        '兵庫県尼崎市',     41000,'',  '',  35000,'E食品（株）',''],
    ...
    ['2026/08/28','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','大阪市鶴見区', '奈良県橿原市',     43000,'',  '',  36000,'C物流（株）',''],
    ...
    ['2026/08/31','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','吹田市',        '京都市伏見区',     52000,2000,1500,44000,'C物流（株）',''],
    ...
    ['2026/08/31','E食品傭車','20トレ','大阪き503','西村 幸平','090-7777-8888',  '大阪市住之江区','兵庫県西宮市',         55000,2000,2000,46000,'E食品（株）',''],
    ['2026/08/31','A運輸専属','4tW','大阪か101','水島 建二','090-1111-2222', '堺市堺区',   '京都市南区',       47000,1500,1500,40000,'A運輸（株）','専属'],
    ['2026/08/31','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','摂津市',       '奈良県生駒市',         49000,1000,1000,41000,'C物流（株）',''],
    ...
    ['2026/09/01','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','大阪市住之江区','兵庫県西宮市',         55000,2000,2000,46000,'C物流（株）',''],
    ...
    ['2026/09/01','E食品傭車','20トレ','大阪き503','西村 幸平','090-7777-8888',  '守口市',        '神戸市灘区',       38000,1000,1000,32000,'E食品（株）',''],
    ['2026/09/01','A運輸専属','4tW','大阪か101','水島 建二','090-1111-2222', '大阪市鶴見区', '滋賀県彦根市',             62000,3000,3000,52000,'A運輸（株）','専属'],
    ['2026/09/01','C物流傭車','13平','大阪か310','松本 里佳','090-3333-4444','守口市',        '京都市伏見区', 48000,1500,1500,40000,'C物流（株）',''],
...
// ================================================================
//  14-11: 傭車履歴取得（getYoushaHistory）  【大A / 中14 / 小14-11】
//  車番連絡ダイアログの「履歴」ボタンから google.script.run 経由で呼ばれる
// ================================================================
function getYoushaHistory(clientName, ssId) {
  if (!ssId) return [];
  var ss;
  try { ss = SpreadsheetApp.openById(ssId); } catch(e) { return []; }
  if (!ss) return [];
  var sh = ss.getSheetByName('傭車履歴');
  if (!sh || sh.getLastRow() < 2) return [];
  var data = sh.getRange(2, 1, sh.getLastRow() - 1, 6).getValues();
  var results = [];
  data.forEach(function(r) {
...
// ================================================================
//  14-12: 傭車履歴保存・公開ラッパー（saveYoushaHistory）  【大A / 中14 / 小14-12】
//  印刷・メール・FAX送信時に HTML から直接呼ばれる
// ================================================================
function saveYoushaHistory(clientName, yousha, ssId) {
  if (!ssId) return;
  var ss;
  try { ss = SpreadsheetApp.openById(ssId); } catch(e) { return; }
  if (!ss) return;
  saveYoushaHistory_(clientName, yousha, ss);
}

// ================================================================
//  14-12i: 傭車履歴保存・内部実装（saveYoushaHistory_）  【大B / 中14 / 小14-12i】
//  重複チェック: 荷主名＋看板名＋車番＋乗務員名が同一なら最終使用日を更新のみ
// ================================================================
function saveYoushaHistory_(clientName, youshaData, ss) {
  if (!ss) return;
