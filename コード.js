...
// ■ シート構成
//   設定              : A列=トン数, B列=基準燃費
//   運行(24列)        : ID,区分,会社名,トン数,車種,車番,乗務員名,携帯番号,
//                       日付,荷主名,積地,降地,積完時刻,休憩開始,休憩終了,降完時刻,
//                       売上,請求高速,実費高速,合計高速,連絡事項,データ,連絡端末,データ端末
//   集計表(30列)      : ID,区分,会社名,トン数,車種,車番,乗務員名,携帯番号,
//                       日付,荷主,積地,降地,積完時刻,休憩開始,休憩終了,降完時刻,
...
//  積完・休憩開始・休憩終了・降完（M〜P列=13〜16）に書式をセットする
// ================================================================
function applyDateTimeFormat_(sheet, startRow, numRows) {
  if (numRows <= 0) return;
  var fmt = 'M/d HH:mm';
  var cols = [13, 14, 15, 16];
  for (var i = 0; i < cols.length; i++) {
    sheet.getRange(startRow, cols[i], numRows, 1).setNumberFormat(fmt);
  }
}
...
function onEditUnkou_(sheet, range) {
  ...
  for (var i = 0; i < numRows; i++) {
    ...
    // M/N/O/P列（積完・休憩・降完時刻）：全角文字・日付なし時刻を正規化して合成
    if ([13, 14, 15, 16].indexOf(editedCol) !== -1) {
    ...
    var tollCell = sheet.getRange(row, 20);
    if (!tollCell.getFormula()) {
      tollCell.setFormula('=IF(AND(S' + row + '="",R' + row + '=""),"",S' + row + '-R' + row + ')');
    }
...
function generateSummary() {
  ...
  if (sumSheet && sumSheet.getLastRow() >= 2) {
    ...
    var oldRows = sumSheet.getRange(2, 1, sumSheet.getLastRow()-1, Math.max(colCount, 30)).getValues();
    for (var o = 0; o < oldRows.length; o++) {
      var oldId = String(oldRows[o][0] || '').trim();
      if (oldId) {
        oldData[oldId] = {
          distance: oldRows[o][20],
          gas:      oldRows[o][22],
          pay:      oldRows[o][24],
          memo:     oldRows[o][26],
          kari:     oldRows[o][27] || '',
  ...
  for (var i = 1; i < unkouData.length; i++) {
    ...
    if (r[12] && !g.pickTime)  { g.pickTime  = r[12]; g.rawPickTime  = new Date(r[12]); }
    if (r[13] && !g.restStart) { g.restStart = r[13]; g.rawRestStart = new Date(r[13]); }
    if (r[14] && !g.restEnd)   { g.restEnd   = r[14]; g.rawRestEnd   = new Date(r[14]); }
    if (r[15] && !g.dropTime)  { g.dropTime  = r[15]; g.rawDropTime  = new Date(r[15]); }
    g.sales   += Number(r[16]) || 0;
    g.tollReq += Number(r[17]) || 0;
    g.tollReal+= Number(r[18]) || 0;
  ...
  for (var o2 = 0; o2 < idOrder.length; o2++) {
    ...
    outRows.push([
      ...
      g.pickTime, g.restStart, g.restEnd, g.dropTime,
  ...
  sumSheet.clear();
  if (outRows.length > 0) {
    sumSheet.getRange(1, 1, outRows.length, 30).setValues(outRows);
    ...
    for (var row = 2; row <= outRows.length; row++) {
      sumSheet.getRange(row, 20).setFormula('=IF(AND(S'+row+'="",R'+row+'=""),"",S'+row+'-R'+row+')');
      sumSheet.getRange(row, 24).setFormula('=IF(OR(U'+row+'="",V'+row+'=""),"",U'+row+'/V'+row+'*W'+row+')');
      sumSheet.getRange(row, 26).setFormula('=IF(AND(Q'+row+'="",T'+row+'="",X'+row+'="",Y'+row+'=""),"",Q'+row+'-(T'+row+'+X'+row+'+Y'+row+'))');

      var g2       = idMap[idOrder[row-2]];
      var keepPay  = outRows[row-1][24] || '';
      var keepDist = outRows[row-1][20] || '';
      var keepGas  = outRows[row-1][22] || '';
      ...
      sumSheet.getRange(row, 1, 1, 30).setBackground(calcProfit < 0 ? '#ffebee' : null);
      sumSheet.getRange(row, 13, 1, 4).setBackground(null);
      if (g2.rawPickTime  && g2.rawRestStart && (g2.rawRestStart-g2.rawPickTime)  > F) { sumSheet.getRange(row,13,1,2).setBackground('#ffd600'); }
      if (g2.rawRestStart && g2.rawRestEnd   && (g2.rawRestEnd  -g2.rawRestStart) < T) { sumSheet.getRange(row,14,1,2).setBackground('#4fc3f7'); }
      if (g2.rawRestEnd   && g2.rawDropTime  && (g2.rawDropTime -g2.rawRestEnd)   > F) { sumSheet.getRange(row,15,1,2).setBackground('#ffd600'); }
    }
...
function syncSummaryForId_(targetId) {
  ...
  for (var i = 1; i < unkouData.length; i++) {
    ...
    if (!g) { g = { id:String(r[0]).trim(), kubun:r[1], company:r[2], tons:r[3], type:r[4], car:r[5], name:r[6], tel:r[7], date:r[8], client:r[9], picks:[], drops:[], pickTime:'', restStart:'', restEnd:'', dropTime:'', sales:0, tollReq:0, tollReal:0 }; }
    if (r[10]) g.picks.push(r[10]);
    if (r[11]) g.drops.push(r[11]);
    if (r[12] && !g.pickTime)  { g.pickTime  = r[12]; rawPickTime  = new Date(r[12]); }
    if (r[13] && !g.restStart) { g.restStart = r[13]; rawRestStart = new Date(r[13]); }
    if (r[14] && !g.restEnd)   { g.restEnd   = r[14]; rawRestEnd   = new Date(r[14]); }
    if (r[15] && !g.dropTime)  { g.dropTime  = r[15]; rawDropTime  = new Date(r[15]); }
    g.sales   += Number(r[16]) || 0;
    g.tollReq += Number(r[17]) || 0;
    g.tollReal+= Number(r[18]) || 0;
  }
  ...
  if (sumLast >= 2) {
    var colCount = sumSheet.getLastColumn();
    var sumIds   = sumSheet.getRange(2, 1, sumLast-1, Math.max(colCount, 30)).getValues();
    for (var k = 0; k < sumIds.length; k++) {
      if (String(sumIds[k][0]).trim() === String(targetId).trim()) {
        sumRow      = k + 2;
        keepDistance= sumIds[k][20];
        keepGas     = sumIds[k][22];
        keepPay     = sumIds[k][24];
        keepMemo    = sumIds[k][26];
        keepKari    = sumIds[k][27] || '';
        keepKyuryo  = sumIds[k][28] || '';
        keepPct     = sumIds[k][29] || '';
        break;
      }
    }
  ...
  if (sumRow > 0) {
    sumSheet.getRange(sumRow, 1, 1, 30).setValues([rowData]);
  } else {
    sumRow = sumSheet.getLastRow()+1;
    if (sumRow === 1) {
      var hdr = ['ID','区分','会社名','トン数','車種','車番','乗務員名','携帯番号','日付','荷主','積地','降地','積完時刻','休憩開始','休憩終了','降完時刻','売上','請求(高速代)','実費(高速代)','合計(高速代)','距離','燃費','ガソリン代','燃料代','支払い','利益','備考','仮日数','給料','％'];
      sumSheet.getRange(1, 1, 1, 30).setValues([hdr]);
      sumSheet.setFrozenRows(1);
      sumRow = 2;
    }
    sumSheet.getRange(sumRow, 1, 1, 30).setValues([rowData]);
  ...
  sumSheet.getRange(sumRow, 20).setFormula('=IF(AND(S'+sumRow+'="",R'+sumRow+'=""),"",S'+sumRow+'-R'+sumRow+')');
  sumSheet.getRange(sumRow, 24).setFormula('=IF(OR(U'+sumRow+'="",V'+sumRow+'=""),"",U'+sumRow+'/V'+sumRow+'*W'+sumRow+')');
  sumSheet.getRange(sumRow, 26).setFormula('=IF(AND(Q'+sumRow+'="",T'+sumRow+'="",X'+sumRow+'="",Y'+sumRow+'=""),"",Q'+sumRow+'-(T'+sumRow+'+X'+sumRow+'+Y'+sumRow+'))');
  ...
  var F = 4*60*60*1000;
  var T = 30*60*1000;
  sumSheet.getRange(sumRow, 13, 1, 4).setBackground(null);
  if (rawPickTime  && rawRestStart && (rawRestStart-rawPickTime)  > F) { sumSheet.getRange(sumRow,13,1,2).setBackground('#ffd600'); }
  if (rawRestStart && rawRestEnd   && (rawRestEnd  -rawRestStart) < T) { sumSheet.getRange(sumRow,14,1,2).setBackground('#4fc3f7'); }
  if (rawRestEnd   && rawDropTime  && (rawDropTime -rawRestEnd)   > F) { sumSheet.getRange(sumRow,15,1,2).setBackground('#ffd600'); }
...
function calculatePaymentAmount() {
  ...
  for (var i = 0; i < data.length; i++) {
    var rowNum    = i + 2;
    var sales     = Number(data[i][16]) || 0;
    var totalToll = Number(data[i][19]) || 0;
    var kari      = Number(data[i][27]) || 0;
    var kyuryo    = Number(data[i][28]) || 0;
    var pct       = Number(data[i][29]) || 0;
    var yCell     = sheet.getRange(rowNum, 25);

    yCell.setBackground(null);
    sheet.getRange(rowNum, 28, 1, 3).setBackground(null);
    ...
    if (pct > 0) {
      ...
      if (kyuryo > 0 && kari > 0) {
        ...
        if (!kyuryo) sheet.getRange(rowNum, 29).setBackground('#f4cccc');
        if (!kari)   sheet.getRange(rowNum, 28).setBackground('#f4cccc');
      }
    } else {
...
function clearRunState() {
  var p    = PropertiesService.getUserProperties();
  var keys = ['picks','drops','rows','pickDone','dropDone','phase','lastPickRow','pickHistory','dropHistory'];
...
function getTodayRoutes() {
  ...
  for (var i = 1; i < all.length; i++) {
    ...
    out.push({ row: i+1, pick: pickV, drop: dropV, pickDone: !!r[12], dropDone: !!r[15] });
  }
  return out;
}
...
function createParentRows(picks, drops) {
  ...
  try {
    ...
    sheet.getRange(startRow, 1, num, 24).setValues(rowsData);

    var formulas = [];
    for (var i = 0; i < num; i++) {
      var r = startRow + i;
      formulas.push(['=IF(AND(S'+r+'="",R'+r+'=""),"",S'+r+'-R'+r+')']);
    }
    sheet.getRange(startRow, 20, num, 1).setFormulas(formulas);
...
// ================================================================
//  7-4: 休憩開始・終了時刻記録（setRest）
//  type='start'→N列(14), type='end'→O列(15) に現在時刻を書き込む
// ================================================================
function setRest(row, type) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('運行');
  var col = (type === 'start') ? 14 : (type === 'end') ? 15 : 0;
  if (col) {
...
// ================================================================
//  7-5: 降完時刻記録（setDropComplete）
//  指定行のP列（16列目）に現在時刻を書き込み集計表を同期する
// ================================================================
...
function getListData(year, month) {
  ...
  var payMap   = {};
  if (sumSheet && sumSheet.getLastRow() >= 2) {
    var sumAll = sumSheet.getRange(2, 1, sumSheet.getLastRow()-1, 27).getValues();
    for (var s = 0; s < sumAll.length; s++) {
      var sid = String(sumAll[s][0]||'').trim();
      if (sid) payMap[sid] = {
        pay:      Number(sumAll[s][24]) || 0,
        tollTotal:Number(sumAll[s][19]) || 0
      };
    }
  }
  ...
  for (var i = 1; i < all.length; i++) {
    ...
    if (r[11]) g.drops.push(r[11]);
    if (r[12] && !g.pickTime) {
      var pt = r[12] instanceof Date ? r[12] : new Date(r[12]);
      if (!isNaN(pt.getTime())) {
  ...
  for (var o = 0; o < idOrder.length; o++) {
    ...
    result.push({
      ...
      pickTime:g.pickTime, restStart:g.restStart, restEnd:g.restEnd, dropTime:g.dropTime,
      ...
      isNew:      !g.pickTime && !g.restStart && !g.restEnd && !g.dropTime
    });
...
function getEditData(id) {
  ...
  if (sumSheet && sumSheet.getLastRow() >= 2) {
    ...
    for (var s = 0; s < sumAll.length; s++) {
      if (String(sumAll[s][0]||'').trim() === String(id).trim()) {
        sumData.tollTotal = sumAll[s][19] !== '' ? sumAll[s][19] : '';
        sumData.profit    = sumAll[s][25] !== '' ? sumAll[s][25] : '';
        break;
      }
    }
  }
  ...
  var pickTime='', restStart='', restEnd='', dropTime='';
  ...
  for (var i = 1; i < all.length; i++) {
    ...
    // 売上・高速は合算
    totalSales   += Number(r[16]) || 0;
    totalTollReq += Number(r[17]) || 0;
    totalTollReal+= Number(r[18]) || 0;

    // 時刻は最初に見つかった値を使用
    if (!pickTime  && r[12]) pickTime  = Utilities.formatDate(new Date(r[12]),'Asia/Tokyo','HH:mm');
    if (!restStart && r[13]) restStart = Utilities.formatDate(new Date(r[13]),'Asia/Tokyo','HH:mm');
    if (!restEnd   && r[14]) restEnd   = Utilities.formatDate(new Date(r[14]),'Asia/Tokyo','HH:mm');
    if (!dropTime  && r[15]) dropTime  = Utilities.formatDate(new Date(r[15]),'Asia/Tokyo','HH:mm');
  }
  ...
  return {
    ...
    notice:   baseData[20] || '',
    dataUrl:  baseData[21] || '',
    termNotice:baseData[22]|| '',
    termData: baseData[23] || '',
...
function saveEditData(obj) {
  ...
  for (var i = 1; i < all.length; i++) {
    ...
    if (obj.pickTime)  { var c13=sheet.getRange(r,13); c13.setValue(new Date(obj.date+' '+obj.pickTime));  c13.setNumberFormat(timeFmt); }
    else               sheet.getRange(r, 13).clearContent();
    if (obj.restStart) { var c14=sheet.getRange(r,14); c14.setValue(new Date(obj.date+' '+obj.restStart)); c14.setNumberFormat(timeFmt); }
    else               sheet.getRange(r, 14).clearContent();
    if (obj.restEnd)   { var c15=sheet.getRange(r,15); c15.setValue(new Date(obj.date+' '+obj.restEnd));   c15.setNumberFormat(timeFmt); }
    else               sheet.getRange(r, 15).clearContent();
    if (obj.dropTime)  { var c16=sheet.getRange(r,16); c16.setValue(new Date(obj.date+' '+obj.dropTime));  c16.setNumberFormat(timeFmt); }
    else               sheet.getRange(r, 16).clearContent();

    // 売上・高速は最初の行のみ書き込む（複数行IDの場合の重複防止）
    if (!written) {
      sheet.getRange(r, 17).setValue(obj.sales   || '');
      sheet.getRange(r, 18).setValue(obj.tollReq || '');
      sheet.getRange(r, 19).setValue(obj.tollReal|| '');
      written = true;
    } else {
    ...
    sheet.getRange(r, 23).setValue(obj.termNotice || '');
  }
...
function saveNotice(id, text) {
  ...
  for (var i = 1; i < all.length; i++) {
    if (String(all[i][0]||'').trim() === String(id).trim()) {
      sheet.getRange(i+1, 21).setValue(text); break;
    }
  }
}
...
function uploadFile(id, fileName, base64Data, mimeType) {
  ...
  if (sheet) {
    ...
    for (var i = 1; i < all.length; i++) {
      if (String(all[i][0]||'').trim() === String(id).trim()) {
        sheet.getRange(i+1, 22).setValue(url); break;
      }
    }
  }
...
function uploadTerminalFile(id, fileName, base64Data, mimeType) {
  ...
  if (sheet) {
    ...
    for (var i = 1; i < all.length; i++) {
      if (String(all[i][0]||'').trim() === String(id).trim()) {
        sheet.getRange(i+1, 24).setValue(url); break;
      }
    }
  }
  return { ok: true, url: url, fileName: fileName };
...
function getRoutesById(id) {
  ...
  for (var i = 1; i < all.length; i++) {
    if (String(all[i][0]||'').trim() !== String(id).trim()) continue;
    var pDone = !!all[i][12], dDone = !!all[i][15];
    var pickVal = (all[i][10] instanceof Date) ? '' : String(all[i][10] || '');
    var dropVal = (all[i][11] instanceof Date) ? '' : String(all[i][11] || '');
    routes.push({ row:i+1, pick:pickVal, drop:dropVal, pickDone:pDone, dropDone:dDone });
    if (!pDone) allPickDone = false;
    if (pDone)  anyPickDone = true;
    if (all[i][13]) hasRestS = true;
    if (all[i][14]) hasRestE = true;
