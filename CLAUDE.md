...
## 報告の仕方（トークン節約・必須ルール）
- 作業前の「確認しますか？」「合ってますか？」は禁止。指示が来たら即実行してdiffを出す
- 不明点は1行で質問する（長文で経緯を説明しない）
- 報告は結論1行＋必要最小限の情報のみ。締め文句（「〜しました。よろしいでしょうか？」等）は全部禁止
- diffは変更箇所のみ。変更しない行は省略記号（...）で省略する
- 正常完了時は「完了」の1行だけ。エラーや問題が起きた時だけ詳しく報告する
- grepや確認作業の結果は該当行のみ出力する。全文出力しない
- 長い説明・背景・経緯の説明は不要。聞かれてないことは書かない

## セッション管理
- 1つの作業が完了したら即座に引き継ぎメモ.mdを更新して止まる
- 会話が長くなってきたら「引き継ぎメモ更新しました。新規セッション推奨」の1行だけ出して止まる
- 新規セッション開始時はCLAUDE.mdと引き継ぎメモ.mdだけ読む。それ以外の過去の会話は読まない

## ■ 修正対象外ファイルへの対応
- 作業中に指示対象外のファイルで問題を発見しても、その場でEditを実行しない
- デプロイ失敗・エラー等で「指示外ファイル」に手を入れる必要が生じた場合、以下の順番を必ず守る：
  1. 何が起きているか（現象・原因）を報告する
  2. どう直すか（具体的な変更内容）を提示する
  3. 「修正してよいですか？」と明示的に確認を求めて止まる
  4. ユーザーのOKが出るまでEdit/Writeツールを実行しない
- 「すぐ直せば解決する」「無関係な既存バグだから問題ない」は確認をスキップしていい理由にならない
- 「原因調査 → 修正方針の説明 → 即Edit実行」を1ターンで連続して行うことは禁止。方針提示とEdit実行の間に必ずユーザーの応答を挟む
- 【具体例・違反パターン】2026-07-10: 高-3修正（tryLock）のデプロイ中に check_integrity.js の正規表現誤検知が発覚。原因説明と修正内容を1ターンで書いたままユーザー確認なしに即Edit実行した。「無関係な既存バグ」と自己判断してスキップしたが、これは重大なルール違反。
